...
function createOrUpdateCanvasMatrix(gridSize) {
    ...
    for (var i = 0; i < gridSize; i++) {
        ...
        for (var k = 0; k < gridSize; k++) {
            ...
            canvasElement.setAttribute('pieceid', String(pieceId++));
            $container.append(canvasElement).isotope('appended', canvasElement);
        }
    }
}

function setSortDataOnContainer() {
    $('#localPieceContainer').isotope({
        getSortData: {
            id: function ($elem) {
                return parseInt($elem.getAttribute('pieceid'));
            }
        }
    });
...
function drawPiecesOfVideo() {
    ...
    $('#localPieceContainer').children().each(function () {
        var pieceId = parseInt(this.getAttribute('pieceid'));
        var row = parseInt(pieceId / GRID_SIZE);
        var column = parseInt(pieceId % GRID_SIZE);

        this.getContext('2d').drawImage(localVideo, x * row, y * column,
            x, y, 0, 0, x, y);
    });
}
...
function init() {
    ...
    createOrUpdateCanvasMatrix(GRID_SIZE);
    modifyIsotopeWidth(GRID_SIZE * x, GRID_SIZE * y);
    setSortDataOnContainer();
